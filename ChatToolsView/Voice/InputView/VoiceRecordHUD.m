...
//    self.remindLabel.backgroundColor = [UIColor colorWithRed:1.000 green:0.000 blue:0.000 alpha:0.630];
    self.remindLabel.text = NSLocalizedStringFromTable(@"手指松开,取消发送", @"MessageDisplayKitString", nil);
}
...
- (void)configRecordingHUDImageWithPeakPower:(CGFloat)peakPower {
    NSString *imageName = @"ChatTool.bundle/RecordingSignal00";
    if (peakPower >= 0 && peakPower <= 0.1) {
        imageName = [imageName stringByAppendingString:@"1"];
    } else if (peakPower > 0.1 && peakPower <= 0.299) {
    ...
    } else if (peakPower > 0.5 && peakPower <= 0.699) {
        ...
        imageName = [imageName stringByAppendingString:@"8"];
        NSLog(@"%f currentThread: %@",peakPower, [NSThread currentThread]);
    }
...
- (void)setup {
    ...
    if (!_microPhoneImageView) {
        ...
        microPhoneImageView.image = [UIImage imageNamed:@"ChatTool.bundle/RecordingBkg"];
    ...
    if (!_recordingHUDImageView) {
        ...
        recordHUDImageView.image = [UIImage imageNamed:@"ChatTool.bundle/RecordingSignal001"];
    ...
    if (!_cancelRecordImageView) {
        ...
        cancelRecordImageView.image = [UIImage imageNamed:@"ChatTool.bundle/RecordCancel"];
